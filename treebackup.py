...
class TreeBackup(Agent):
  def __init__(self, mdp, init_state=None, steps=1):
    super(TreeBackup, self).__init__(mdp, init_state)
    self._n = steps

  def episode(self, discount, step_size, max_steps=1e3):
    """ Run n-step TreeBackup for one episode.
        Algorithm follow the pseudo-code on page 159 of the RL textbook.
    """
    self._T = float('inf')
    self._tau = 0
  ...
  def act(self, action, discount, step_size):
    """ take an action and update Q given the discount factor and step size """
    if self._time < self._T:
        ...
        if self._env.terminal(sp): # if terminal
            ...
            self._delta[self._time%self._n] = r - self._Qt[self._time%(self._n+1)] + discount * self.expected_Q(sp)
    ...
    if self._tau >= 0:
        ...
        for k in range(self._tau, int(min(self._time, self._T-1))+1):
            ...
            E *= discount * self._pi[(k+1)%self._n]
...
def example():
  ...
  # create n-step TreeBackup agent
  agent = TreeBackup(mdp, 0, 3)
  agent2 = TreeBackup(mdp, 0, 3)
  ...
  print('Equiprobable Q_TB[s][a]', agent.Q)
  print('Eps greedy   Q_TB[s][a]', agent2.Q)
